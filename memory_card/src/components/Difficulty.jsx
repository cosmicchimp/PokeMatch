...
export function Difficulty({ updateGameStart, updateDiffLevel }) {
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "ArrowDown") {
        updateActiveOption((prev) => (prev + 1) % levels.length); // Move down
  ...
  }, [activeOption, updateGameStart]);

  return (
    <div className="diffWrapper">
      <h2 className="diffModalTitle">Please choose a difficulty level</h2>
      ...
      <div className="githubRepo" onClick={githubLink}>
        <svg
          className="svgIcon"
        ...
        <div className="githubTitle"> Github Repo</div>
      </div>
    </div>
  );
}
